...
const activePreviews = new Map<string, { projectId: string; tenantId: string; status: string; createdAt: Date; expiresAt: Date }>();
...
app.post('/api/previews', (req, res) => {
  const { projectId, tenantId, mode, durationMinutes } = req.body;
  const previewId = `preview_${Date.now()}`;
  const now = new Date();
  const expiresAt = new Date(now.getTime() + (durationMinutes || 30) * 60 * 1000);
...
app.get('/preview/:id', (req, res) => {
  const preview = activePreviews.get(req.params.id);
  if (!preview) {
    res.status(404).send('<html><body><h1>Preview not found</h1></body></html>');
    return;
  }
  res.send(`
    <!DOCTYPE html>
